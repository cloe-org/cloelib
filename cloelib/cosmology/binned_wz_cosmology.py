...
class DEBackground:
    ...
    def __init__(self, H0: float, Omega_b0: float, Omega_cdm0: float, Omega_k0: float,
                 ...
                 w_i: np.ndarray, binning: str = "step") -> None:
        """
    ...
    def hubble_parameter(self, zs: np.ndarray, units: str = "km/s/Mpc") -> np.ndarray:
        ...
        """
        DE_z_grid = get_de_density(zs, self.zbin_edges, self.zbin_widths, self.zbin_centers, self.w_i)
        E_z_grid = np.sqrt(self.Omega_m0*pow(1.+zs, 3) + (1-self.Omega_m0)*DE_z_grid)
        if units == "1/Mpc":
    ...
    def comoving_distance(self, zs: np.ndarray) -> np.ndarray:
        ...
        """
        r_z_int = lambda z: 1./np.sqrt(self.Omega_m0*pow(1.+z, 3) + (1-self.Omega_m0)*get_de_density_i(z, self.zbin_edges, self.zbin_widths, self.zbin_centers, self.w_i))
        zz_ = np.hstack(([0.], zs)) if zs[0] != 0. else zs
        r_z_grid = np.array([quad(r_z_int, zz_[i], zz_[i+1])[0]/self.H0 for i in range(len(zz_)-1)])*SPEED_OF_LIGHT/1000 #Mpc
    ...
    def transverse_comoving_distance(self, zs: np.ndarray) -> np.ndarray:
        ...
        if self.Omega_k0 == 0.0:
            ...
            raise ValueError("Omega_k0 != 0.0 not supported for DEBackground.transverse_comoving_distance.")
    ...
    def Omega_m(self, zs: np.ndarray) -> np.ndarray:
        ...
        """
        if np.isscalar(zs):
            DE_z_grid = get_de_density_i(zs, self.zbin_edges, self.zbin_widths, self.zbin_centers, self.w_i)
        else:
            DE_z_grid = get_de_density(zs, self.zbin_edges, self.zbin_widths, self.zbin_centers, self.w_i)
        E2_z_grid = self.Omega_m0*pow(1.+zs, 3) + (1-self.Omega_m0)*DE_z_grid
        return self.Omega_m0*pow(1.+zs, 3)/E2_z_grid
    ...
    def Omega_b(self, zs: np.ndarray) -> np.ndarray:
        ...
        if np.isscalar(zs):
            ...
            DE_z_grid = get_de_density(zs, self.zbin_edges, self.zbin_widths, self.zbin_centers, self.w_i)
        E2_z_grid = self.Omega_m0*pow(1.+zs, 3) + (1-self.Omega_m0)*DE_z_grid
        return self.Omega_b0*pow(1.+zs, 3)/E2_z_grid
    ...
    def rdrag(self) -> float:
        ...
        rdrag= 55.154*np.exp(-72.3*(w_nu*self.mnu+0.0006)**2)/(Obh2**0.12807 * (Obh2+Och2)**0.25351)
        return rdrag

class DELinearPerturbations:
    """Class for linear perturbations using MGrowth, inheriting from Perturbations parent class."""

    def __init__(self, background: DEBackground, base_linear_perturbations: Perturbations) -> None:
        assert background.Omega_k0 == 0, 'Non flat geometries not supported'
        # must be binned-DE
        self.background = background
        # must be LCDM
        assert base_linear_perturbations.background.w0==-1.0 and base_linear_perturbations.background.wa==0.0, 'Base linear perturbations must be LCDM'
        ...
        self.a_grid = jnp.linspace(1e-3, 1.0, 512)
        z_grid = 1./np.linspace(1e-3, 1.0, 512)- 1.
    ...
    def _compute_growth(self, bg_dict):
        ...
        D_mg, f_mg = np.repeat(D_raw[::-1, None], self.k_len, axis=1), np.repeat(f_raw[::-1, None], self.k_len, axis=1)  
        ...
        self.dz_norm_lcdm_interp = interpolate.RectBivariateSpline(self.z_sorted, self.k, D_mg/D_base[0, None], kx=1, ky=1)
        self.dz_norm_lcdm_z_interp = interpolate.RectBivariateSpline(self.z_sorted, self.k, D_mg/D_base[:, None], kx=1, ky=1)
...
class DENonlinearPerturbations:
    ...
    def __init__(self, background : DEBackground,
        ...
        self.params_hm_emu = {
            ...
            'As': self.background.As*self.de_lin.dz_norm_lcdm_z_interp(self.z, np.full(len(self.z), 0.01))[:, 0]**2, 
        ...
        for key in self.params_hm_emu.keys():
            if key != 'As':
                ...
                check = np.array([np.prod(As_i - hm_bounds['As']) for As_i in self.params_hm_emu['As']])
                if (check > 0).any():
        ...
        Pk_lin = linearperturbations.matter_power_spectrum(linearperturbations.z, linearperturbations.k)[Pk_lin_mask_z][:,Pk_lin_mask_k]
