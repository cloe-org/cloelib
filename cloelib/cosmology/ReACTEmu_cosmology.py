...
class MGemuNonlinearBoost:
    def __init__(self, background : Background,
        ...
        bounds = BOUNDS_BY_MODEL.get(self.gravity_model, BOUNDS_BY_MODEL.get(backend_model, {}))
        ...
        Omega_nu = self.background.mnu / 93.14 / (self.background.H0 / 100.0) ** 2
        Omega_m  = self.background.Omega_cdm0 + self.background.Omega_b0 + Omega_nu
        ...
        if high_z_policy == "power_law":
            ...
            if high_z_policy == "one":
                ...
                boost_out[idx_hi, :] = 1.0 + (last - 1.0)[None, :] * np.exp(-dz / z_decay)
