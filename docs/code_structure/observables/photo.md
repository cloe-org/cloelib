...
- Assumes a `WeylNonLinearPerturbations` or `WeylLinearPerturbations` instance
...
- Modifies the clustering window with the Weyl growth normalization and `sigma8_ini`
...
- Modifies the clustering window with `Jhat`, `bhat`, `Omega_m(z)^{-1}`, and `sigma8_ini^2`
