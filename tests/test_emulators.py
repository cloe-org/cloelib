...
from cloelib.cosmology.cosmopower_cosmology import (
    w0waCDM_Linear, wCDM_Linear, LCDM_Linear,
    w0waCDM_Pcb_Linear, wCDM_Pcb_Linear, LCDM_Pcb_Linear
)
...
def background_w0wa():
    ...
    return CAMBBackground(
        ...
        Omega_k0=Omega_k0, As=As, ns=ns, mnu=mnu, N_mnu=N_mnu,
        w0=w0, wa=wa, gamma_MG=0.0
    )
...
def test_lcdm_growth_factor_normalization(background_lcdm, z_array, k_array):
    ...
    np.testing.assert_allclose(D_z0, 1.0, rtol=1e-6,
                               err_msg="Growth factor should be 1 at z=0")
...
def test_parameter_out_of_bounds():
    ...
    bad_background = CAMBBackground(
        H0=150, Omega_b0=Omega_b0, Omega_cdm0=Omega_cdm0,
        Omega_k0=Omega_k0, As=As, ns=ns, mnu=mnu, N_mnu=N_mnu,
        w0=w0, wa=wa, gamma_MG=0.0
    )
...
def test_non_flat_geometry():
    ...
    curved_background = CAMBBackground(
        ...
        As=As, ns=ns, mnu=mnu, N_mnu=N_mnu,
        w0=w0, wa=wa, gamma_MG=0.0
    )
...
def test_pcb_vs_total_matter(background_lcdm, z_array, k_array):
    ...
    np.testing.assert_allclose(pk_total, pk_cb, rtol=0.01,
                               err_msg="Pcb and total should match when mnu=0")
