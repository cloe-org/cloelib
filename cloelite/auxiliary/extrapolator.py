...
def extend_spectra(wavenumber_in, redshift_in, boost_in,
    ...
    # same thing.
        ...
        redshift_plus = \
            redshift_base[redshift_base > redshift_in[-1]]

        # Join them together for output
        redshift_out = np.concatenate((redshift_in,
                                       redshift_plus))
        boost_out = np.ones((len(redshift_out),
                             len(wavenumber_out)))

        # Use correct result in the range of the emulator in redshift and
        # wavenumber
        boost_out[:len(redshift_in),
        ...
        if len(redshift_plus)>0:
            ...
            if option_redshift == "hm_simple":
                ...
                boost_out[len(redshift_in):,
                          len(wavenumber_minus):(len(wavenumber_minus) +
                          len(wavenumber_in))] = \
                    extrap_func(redshift_in=redshift_plus,
                                wavenumber_in=wavenumber_in)[2]
                ...
                n_extra_b = \
                    ...
                    / (np.log(1 + redshift_in[-1]) -
                       np.log(1 + redshift_in[-2]))
            ...
            if option_wavenumber == "const":
                ...
                boost_out[:, (len(wavenumber_minus) +
                    ...
                    extrap_func(
                        ...
                        wavenumber_in=wavenumber_plus)[2]
                ...
                tanh_scale = wavenumber_tanh_scale \
                ...
                k3Pk = wavenumber_in[-1] ** (4 - ns) * \
                    boost_out[:, i_last - 1]

                dP_dlogk = \
                    (wavenumber_in[-1] ** (4 - ns) *
                     boost_out[:, i_last - 1] -
                     wavenumber_in[-2] ** (4 - ns) *
                     boost_out[:, i_last - 2]) / \
                    (np.log(wavenumber_in[-1]) - np.log(wavenumber_in[-2]))
                ...
                logkstar_logk2 = \
                    np.log(wavenumber_in[-1]) - 2 * k3Pk / dP_dlogk

                boost_out[:, (len(wavenumber_minus) +
                              len(wavenumber_in)):] = \
                    wavenumber_plus[None, :] ** - (4 - ns) * \
                    Amp_logk2[:, None] * \
                    (np.log(wavenumber_plus[None, :]) -
                     logkstar_logk2[:, None]) ** 2
        ...
        if (wavenumber_base[0] < wavenumber_in[0]) & (option_wavenumber == "logk2"):
            ...
            n_extra_b = (np.log(boost_out[:, i_first+1]) -
                         np.log(boost_out[:, i_first])) / \
        ...
        if (not flag_range) and option_cosmo == "hm_smooth":
            ...
            boost_hmcode = \
                extrap_func(redshift_in=redshift_out,
