...
class MGrowthLinearPerturbations:
    ...
    def __init__(self, 
        ...
        if self.gravity_model == 'musigma-de':
            if 'mu0' in mgpars and 'sigma0' in mgpars:
                self.mu_interp = self._compute_mu_de_interp(mgpars['mu0'], background_mgrowth['Omega_m'], background_mgrowth['w0'], background_mgrowth['wa'])
                self.sigma_lensing = self._compute_sigma_de_interp(mgpars['sigma0'], background_mgrowth['Omega_m'], background_mgrowth['w0'], background_mgrowth['wa'])
        ...
        if self.gravity_model == 'mu':
            if 'mu0' in mgpars and 'sigma0' in mgpars and 'c1' in mgpars and 'c2' in mgpars and 'lam' in mgpars:
                self.mu_interp = self._compute_mu_de_scaledep_interp(mgpars['mu0'], mgpars['c1'], mgpars['lam'], background_mgrowth['Omega_m'], background_mgrowth['w0'], background_mgrowth['wa'])
                self.sigma_lensing = self._compute_sigma_de_scaledep_interp(mgpars['sigma0'], mgpars['c2'], mgpars['lam'], background_mgrowth['Omega_m'], background_mgrowth['w0'], background_mgrowth['wa'])
                # If mu0 > 2*sigma0+1, MGCAMB breaks but MGrowth does not;
                # so it depends on your setup, by default this condition 
                # is imposed because nonlinear emulators used MGCAMB.
                if mgpars['mu0']>2.*mgpars['sigma0']+1.:
        ...
        if self.gravity_model == 'musigma-de':
            ...
            self._compute_growth_generic = getattr(self, '_compute_growth_muinterp_scaledep')
        else:
    ...
    def _compute_mu_de_interp(self, mu0, omega0, w0, wa):
        omegaL = (1.-omega0) * self.a_interp**(-3.*(1.+w0+wa)) * np.exp(-3.*wa*(1.-self.a_interp))
        omegaL0 = (1.-omega0) 
        E2 = omega0/self.a_interp**3 + omegaL
        mu_de = 1. + mu0*(omegaL/E2)/omegaL0
    ...
    def _compute_mu_de_scaledep_interp(self, mu0, c1, lam, omega0, w0, wa):
        ...
        mu_de_k = np.array([
        1. + mu0*(omegaL/E2)/omegaL0
            * (1.+c1*(lam*E/k_i)**2) / (1.+(lam*E/k_i)**2)
        for k_i in k_arr
        ])
        mu_interpolator = [
            interpolate.interp1d(self.a_interp, row, bounds_error=False,
                                kind='cubic', fill_value=(row[0], row[-1]))
            for row in mu_de_k
        ]
    ...
    def _compute_sigma_de_interp(self, sigma0, omega0, w0, wa):
        omegaL = (1.-omega0) * (1.+self.z_sorted)**(3.*(1.+w0+wa)) * np.exp(-3.*wa*self.z_sorted/(1.+self.z_sorted))
        omegaL0 = (1.-omega0) 
        E2 = omega0*(1.+self.z_sorted)**3 + omegaL
        sigma_de = 1. + sigma0*(omegaL/E2)/omegaL0
        ...
        sigma_interpolator = interpolate.RectBivariateSpline(self.z_sorted, self.k, sigma_de, kx=1, ky=1)
    ...
    def _compute_sigma_de_scaledep_interp(self, sigma0, c2, lam, omega0, w0, wa):
        ...
        # Note that we use the same convention as in ReACTEmu_cosmology, to convert to DESI parametrisation
        # lam = lam_DESI/2997.92458
        sigma_de = 1. + sigma0*(omegaL[z_ax]/E2[z_ax])/omegaL0 \
                    * (1.+c2*(lam*E[z_ax]/k_arr[k_ax])**2) \
                    / (1.+(lam*E[z_ax]/k_arr[k_ax])**2)
    ...
    def _compute_growth_ds(self):   
        ...
        omega0 = omega_cdm + self.background.Omega_b0 + self.background.mnu/93.14/h**2
        Rc = omega_cdm/omega0
        unit_conv = 0.0194407
        xi = self.mgpars['xi']
        w0 = self.background.w0
        # approximation from Carrilho et al. 2021 https://arxiv.org/pdf/2111.13598
        xi_corrected = xi*Rc/(1.+ unit_conv*h*(1.-omega0)*(1. + w0)*xi*(1.-Rc)) 
    ...
    def _compute_growth_muinterp_scaledep(self):  
        d_f_i =  [self.mg_cosmo.growth_parameters(mu_interp=mu_interpolator_k_i) for mu_interpolator_k_i in self.mu_interp]
    ...
    def _compute_growth(self, bg_dict):
        ...
        self.dz_norm_w0wacdm_interp = interpolate.RectBivariateSpline(self.z_sorted, self.k, D_mg/D_base[0, None], kx=1, ky=1)
    ...
    def sigma8_0(self) -> float:
        ...
        if self.gravity_model == 'fr' or self.gravity_model == 'mu':
            ...
            pk_lin_z0 = self.matter_power_spectrum(0., ks)
            bes_j_1 = lambda x: np.sin(x)/x - np.cos(x)
            return np.sqrt(integrate.simpson((3*bes_j_1(ks*8)/(ks*8)**2)**2*ks**2*pk_lin_z0/2/np.pi**2,ks))
        else:
            return self.dz_norm_w0wacdm_interp(0., 0.01)[0, 0]*self.base.sigma8_0()