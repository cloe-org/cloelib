...
- wCDM with three degenerate massive neutrinos
...
zenodo_path="https://zenodo.org/records/16753034/files"
...
class w0waCDM_3degen_Linear:
    ...
    def __init__(self, background : Background, redshifts: np.ndarray):
        ...
        cp_bounds = {'ombh2': np.array([0.001, 0.1]),
...
class wCDM_3degen_Linear:
    """
    Emulator for the linear matter power spectrum in the wCDM cosmology with three degenerate massive neutrinos.

    This class uses a Cosmopower-trained neural network to emulate the linear power spectrum 
    for a wCDM cosmology with three degenerate neutrinos. Neutrinos are modeled as in Archidiacono et al. (2024). The total mass
    sum is described by the `mnu` parameter.
    """
...
class LCDM_3degen_Linear:
    ...
    This class uses a Cosmopower-trained neural network to emulate the linear power spectrum 
    for a LCDM cosmology with three degenerate neutrinos. Neutrinos are modeled as in Archidiacono et al. (2024). The total mass
    sum is described by the `mnu` parameter.
    """
...
class w0waCDM_3degen_Pcb_Linear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] linear matter power spectrum in the w0waCDM cosmology with three degenerate massive neutrinos.
    ...
    def __init__(self, background : Background, redshifts: np.ndarray):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-3degen-pcb-linear-spectra.pkl", zenodo_path)
            self.cp_LIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class w0waCDM_1mass_Pcb_Linear:
    """
...
class wCDM_3degen_Pcb_Linear:
    ...
    def __init__(self, background : Background, redshifts: np.ndarray):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-3degen-pcb-linear-spectra.pkl", zenodo_path)
            self.cp_LIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class LCDM_3degen_Pcb_Linear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] linear matter power spectrum in the LCDM cosmology (w is set to -1) with three degenerate massive neutrinos.
...
class w0waCDM_3degen_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the w0waCDM cosmology with three degenerate massive neutrinos.

    This class uses a Cosmopower-trained neural network to emulate the nonlinear power spectrum 
    for a w0waCDM cosmology with three degenerate neutrinos. Neutrinos are modeled as in Archidiacono et al. (2024). The total mass
    sum is described by the `mnu` parameter. Nonlinear corrections are applied using the mead2020 model in CAMB, with baryonic feedback regulated using the `logT_AGN` parameter.
    """
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-3degen-nonlinear-spectra.pkl", zenodo_path)
        ...
        k_out, z_out, Pk_out = extend_spectra(self.k_emu, self.z , Pk_nonlin,flag_range=True,
                           option_wavenumber="logk2",
                           option_redshift="power_law", extrap_z = redshifts,
                           option_cosmo="const", ns=self.background.ns)
    ...
    def __str__(self):
        ...
        return (
            ...
            f"Neutrinos are modeled as in Archidiacono et al. (2024). There are three "
            f"degenerate massive neutrinos, with a total mass sum described by `mnu` parameter."
            f"Nonlinear corrections are applied using the mead2020 model in CAMB, with baryonic feedback regulated using the `logT_AGN` parameter."
        )
...
class w0waCDM_1mass_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the w0waCDM cosmology with one massive neutrino.

    This class uses a Cosmopower-trained neural network to emulate the nonlinear power spectrum 
    for a w0waCDM cosmology with one massive neutrino. The neutrinos are modeled as in Casas et al. (2023), with the mass of the neutrino controled with `m_nu` parameter.
    Nonlinear corrections are applied using the mead2020 model in CAMB, with baryonic feedback regulated using the `logT_AGN` parameter.
    """
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-1mass-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class w0waCDM_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the w0waCDM cosmology with no massive neutrinos.

    This class uses a Cosmopower-trained neural network to emulate the linear power spectrum 
    for a w0waCDM cosmology with no massive neutrinos (neutrino mass is set to zero). Nonlinear corrections are applied using the mead2020 model in CAMB, with baryonic feedback regulated using the `logT_AGN` parameter.
    """
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class wCDM_3degen_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the wCDM cosmology with three degenerate massive neutrinos.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-3degen-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class wCDM_1mass_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the wCDM cosmology with one massive neutrino.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-1mass-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class wCDM_NonLinear:
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class LCDM_3degen_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the LCDM cosmology (w is set to -1) with three degenerate massive neutrinos.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("3degen-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class LCDM_1mass_NonLinear:
    """
    Emulator for the nonlinear matter power spectrum in the LCDM cosmology (w is set to -1) with one massive neutrino.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("1mass-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def __str__(self):
        ...
        return (
            f"Cosmopower nonlinear Pk module. Computes the nonlinear power spectrum "
            f"for an LCDM cosmology, using input cosmological parameters:\n"
...
class LCDM_NonLinear:
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("lcdm-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def matter_power_spectrum(self,zs,ks):
        ...
        -------
        np.ndarray
            Noninear matter power spectrum in (Mpc/h)^3.
        """
        return self.Pk_int(zs, ks)
...
class w0waCDM_3degen_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the w0waCDM cosmology with three degenerate massive neutrinos.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-3degen-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def matter_power_spectrum(self,zs,ks):
        """
        Compute the cb nonlinear matter power spectrum P_bcn(k, z).
        ...
        -------
        np.ndarray
            Nonlinear cb matter power spectrum in (Mpc/h)^3.
        """
        return self.Pk_int(zs, ks)
...
class w0waCDM_1mass_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the w0waCDM cosmology with one massive neutrino.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-1mass-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class w0waCDM_Pcb_NonLinear:
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("w0wa-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def matter_power_spectrum(self,zs,ks):
        """
        Compute the vb nonlinear matter power spectrum P(k, z).
...
class wCDM_3degen_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the wCDM cosmology with three degenerate massive neutrinos.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-3degen-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class wCDM_1mass_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the wCDM cosmology with one massive neutrino.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("wcdm-1mass-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
...
class wCDM_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] linear matter power spectrum in the wCDM cosmology with no massive neutrinos.
...
class LCDM_3degen_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum for the LCDM cosmology (w is set to -1) with three degenerate massive neutrinos.

    This class uses a Cosmopower-trained neural network to emulate the nonlinear cb power spectrum 
    for a LCDM cosmology with three degenerate neutrinos. Neutrinos are modeled as in Archidiacono et al. (2024). The total mass
    sum is described by the `mnu` parameter. Nonlinear corrections are applied using the mead2020 model in CAMB, with baryonic feedback regulated using the `logT_AGN` parameter.
    """
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("3degen-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def __str__(self):
        """Return emulator description."""
        return (
            f"Cosmopower cb [cold dark matter (c) + baryon (b)] nonlinear Pk module. Computes the nonlinear cb power spectrum "
            f"for an LCDM cosmology with w = -1, using input cosmological parameters:\n"
            f"Inputs: ['ombh2', 'omch2', 'H0', 'ns', 'lnAs', 'z', 'mnu', 'logT_AGN'] \n"
            f"Output: P(k) evaluated between k_min={self.k_min} and k_max={self.k_max}.\n"
            f"Neutrinos are modeled as in Archidiacono et al. (2024). There are three "
...
class LCDM_1mass_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the LCDM cosmology (w is set to -1) with one massive neutrino.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("1mass-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def __str__(self):
        """Return emulator description."""
        return (
            f"Cosmopower cb [cold dark matter (c) + baryon (b)] linear Pk module. Computes the cplinear power spectrum "
            f"for an w0waCDM cosmology, using input cosmological parameters:\n"
            f"Inputs: ['ombh2', 'omch2', 'H0', 'ns', 'lnAs', 'z', 'mnu', 'logT_AGN'] \n"
            f"Output: P(k) evaluated between k_min={self.k_min} and k_max={self.k_max}.\n"
            f"Neutrinos are modeled as in Casas et al. 2023." 
    ...
    def matter_power_spectrum(self,zs,ks):
        """
        Compute the nonlinear cb matter power spectrum P(k, z).
...
class LCDM_Pcb_NonLinear:
    """
    Emulator for the cb [cold dark matter (c) + baryon (b)] nonlinear matter power spectrum in the LCDM cosmology (w is set to -1) with no massive neutrinos.
    ...
    def __init__(self, background : Background, linearperturbations: Perturbations, redshifts: np.ndarray,log10TAGN: Optional[float] = None):
        ...
        with warnings.catch_warnings():
            ...
            cp_file = emulator_data("lcdm-pcb-nonlinear-spectra.pkl", zenodo_path)
            self.cp_NONLIN = cp.cosmopower_NN(restore=True, restore_filename=cp_file)
    ...
    def matter_power_spectrum(self,zs,ks):
        """
        Compute the cb nonlinear matter power spectrum P(k, z).
