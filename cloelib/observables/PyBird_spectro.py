...
try:
    ...
    N = Correlator()
    N.set({'output': 'bPk', 'multipole': 3, 'kmax': 0.5, 
           'km': 1., 'kr': 1., 
           'eft_basis': 'pbj', 
           'with_bias': False,
           'with_stoch': False, 
...
class PyBirdSpectroPower:
    ...
    def __init__(self, linear_perturbations: Perturbations, nuisance_parameters: dict):
        ...
        kk = np.geomspace(1.e-4, 2., 512)
        plin = self.linear_perturbations.matter_power_spectrum(z, kk, hubble_units=True, k_hunit=True)  
        h = self.background.h
        ...
        for c in ['c0', 'c2', 'c4']: nuisance_parameters_pybird[c] *= h**2 # [Mpc]^2 -> [Mpc/h]^2
        for c in ['ct']: nuisance_parameters_pybird[c] *= h**4 # [Mpc]^4 -> [Mpc/h]^4

        pkl = N.get(nuisance_parameters_pybird) # shape (3, Nk) # k: [h/Mpc], Pk: [Mpc/h]^3
        self.ipkl = make_interp_spline(k_ * h, pkl / h**3, k=3, axis=-1)  # k: [1/Mpc], Pk: [Mpc]^3

        pkl_term = N.getmarg(nuisance_parameters_pybird, {'bGamma3', 'c0', 'c2', 'c4', 'ct'}).reshape(-1, 3, k_.shape[0]) # shape (Nterm, Nk*Nl) -> (Nterm, Nl, Nk)
        self.ipkl_term = make_interp_spline(k_ * h, pkl_term / h**3, k=3, axis=-1) # k: [1/Mpc], Pk: [Mpc]^3
    ...
    def rename_nuisance_parameters(self, params):
        ...
        ct_name = (
            "cnlo" if "cnlo" in keys
            else "ck4" if "ck4" in keys
            else None
        )

        bGamma3_name = (
            "bGam3" if "bGam3" in keys
            else "bG3" if "bG3" in keys
            else None
        )

        if ct_name is None:
            raise KeyError("Could not detect whether 'ct' should map to 'cnlo' or 'ck4'.")
        if bGamma3_name is None:
            raise KeyError("Could not detect whether 'bGamma3' should map to 'bGam3' or 'bG3'.")
    ...
    def Pk2d_rsd(self, k: np.ndarray, mu: np.ndarray) -> np.ndarray:
        ...
        """
        leglmu = np.array([legendre(l)(mu) for l in ells])
        if k.ndim == 1: sumrule = 'lk,lm->km'
        elif k.ndim == 2 and k.shape[1] == mu.shape[0]: sumrule = 'lkm,lm->km' # for AP effect, k is a 2D mesh
