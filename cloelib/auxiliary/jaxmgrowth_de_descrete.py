...
def Omega_m(a, omega0, w0, wa):
    ...
    Returns:
        array: Omega_m(a) 
    """
    omegaL = (1.-omega0) * a**(-3.*(1.+w0+wa)) * jnp.exp(3.*(-1.+a)*wa)
    E2 = omega0/a**3 + omegaL
    return omega0/a**3/E2
...
def D_derivatives(D, a, omega0, w0, wa):
    ...
    dlnH = -1.5 * (omega_m_a + (1 + w0 + wa * (1. - a)) * omegaL) / E2
    Om = omega_m_a / E2
...
def IDE_D_derivatives(D, a, omega0=0.3, h=0.68, w0=-1, wa=0, xi=0):
    ...
    dD1 = D2
    dD2 = -D2 / a * (3. + Friction(a, omega0, h, w0, wa, xi) + dlnH) + 1.5 * D1 / a**2 * Om
    return jnp.array([dD1, dD2])
...
def MG_D_derivatives(D, a, a_grid, mu_vals, omega0=0.3, w0=-1, wa=0):
    ...
    dD1 = D2
    dD2 = -D2 / a * (3. + dlnH) + 1.5 * mu_a * D1 / a**2 * Om
    return jnp.array([dD1, dD2])
...
class MGrowth(object):
    def __init__(self, CosmoDict):
        ...
        if CosmoDict is None:
            ...
            self.h = CosmoDict['h']
            self.w0 = CosmoDict['w0'] if 'w0' in CosmoDict else -1.
            self.wa = CosmoDict['wa'] if 'wa' in CosmoDict else 0.
...
class w_a(MGrowth):
    ...
    def growth_parameters(self, a_grid, w_vals, use_fast=True, n_fine=512):
        ...
        if use_fast:
            ...
            D_sol = odeint(DE_D_derivatives_fast, jnp.array([self.a_start, 1.0]), 
                          self.aa, integral_interp, a_fine, a_grid, w_vals, self.omega0)
        else:
            # Original method (slower but kept for comparison)
            D_sol = odeint(DE_D_derivatives, jnp.array([self.a_start, 1.0]), 
                          self.aa, a_grid, w_vals, self.omega0)
        D, dDda = D_sol.T
        f = self.aa * dDda / D
        return D[1:], f[1:]   
...
class mu_a(MGrowth):
    ...
    def growth_parameters(self, a_grid, mu_vals):
        ...
        D, dDda = odeint(MG_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, a_grid, mu_vals, self.omega0, self.w0, self.wa).T  
        f = self.aa*dDda/D
        return D[1:], f[1:]  
class LCDM(MGrowth):
    def __init__(self, CosmoDict=None):
    ...
    def growth_parameters(self, rtol=1.4e-8, atol=1.4e-8):
        ...
        D, dDda = odeint(D_derivatives_LCDM, jnp.array([self.a_start, 1.0]), 
                        self.aa, self.omega0, rtol=rtol, atol=atol).T  
        f = self.aa*dDda/D
        return D[1:], f[1:]
...
class wCDM(MGrowth):
    ...
    def growth_parameters(self, rtol=1.4e-8, atol=1.4e-8):
        ...
        D, dDda = odeint(D_derivatives, jnp.array([self.a_start, 1.0]), 
                        self.aa, self.omega0, self.w0, 0., rtol=rtol, atol=atol).T  
        f = self.aa*dDda/D
        return D[1:], f[1:]
...
class w0waCDM(MGrowth):
    ...
    def growth_parameters(self, rtol=1.4e-8, atol=1.4e-8):
        ...
        D, dDda = odeint(D_derivatives, jnp.array([self.a_start, 1.0]), 
                        self.aa, self.omega0, self.w0, self.wa, rtol=rtol, atol=atol).T  
        f = self.aa*dDda/D
        return D[1:], f[1:] 
...
class IDE(MGrowth):
    ...
    def growth_parameters(self, xi=0.):
        ...
        assert xi>=0
        D, dDda = odeint(IDE_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, self.omega0, self.h, self.w0, self.wa, xi).T  
        f = self.aa*dDda/D
        return D[1:], f[1:]   
class fR_HS(MGrowth):
    def __init__(self, CosmoDict=None):
    ...
    def growth_parameters(self, k_arr, fR0=1e-9):
        ...
        mu_fR = jnp.array([1. + 1./3* var1_i/(var1_i+ ( self.omega0/self.aa_interp**3 - 4.*(self.omega0 -1.) )**3/( 2. * (4. - 3.*self.omega0)**2 * fR0 / c0**2)) for var1_i in var1])
        
        # Pass interpolation arrays directly to MG_D_derivatives for each k
        D_f_i = [odeint(MG_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, self.aa_interp, mu_fR[i, :], self.omega0, -1., 0.).T for i in range(len(k_arr))]
...
class nDGP(MGrowth):
    ...
    def beta(self, a, omegarc):
        ...
        omegaL = (1.-self.omega0) * a**(-3.*(1.+self.w0+self.wa)) * jnp.exp(3.*(-1.+a)*self.wa)
        E = jnp.sqrt(self.omega0/a**3 + omegaL)
        return 1. + E/jnp.sqrt(omegarc)  * (1. + 1./3 * dlnH_dlna(a, self.omega0, self.w0, self.wa))
    ...
    def growth_parameters(self, omegarc=1.e-6):
        ...
        # Pass interpolation arrays directly to MG_D_derivatives
        D, dDda = odeint(MG_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, self.aa_interp, mu_nDGP, self.omega0, self.w0, self.wa).T  
        f = self.aa*dDda/D
        return D[1:], f[1:] 
...
class Linder_gamma(MGrowth):
    ...
    def growth_parameters(self, gamma=0.55, use_quad=True):
        ...
        if use_quad:
            ...
            func = lambda a_: Omega_m(a_, self.omega0, self.w0, self.wa)**gamma/a_
            Dini = self.a_start * hyp2f1(1./3, 1., 11./6, (self.omega0 - 1.) / self.omega0 * self.a_start**3)    
            # Use quad for maximum accuracy (as in original)
            D = jnp.array([(Dini * jnp.exp(quad(func, self.a_start, a_i)[0])) for a_i in self.aa[1:]])
            return D, f 
        else:
            ...
            A = -3.*(1.+self.w0+self.wa);
            omegaf = self.aa_interp**A*jnp.exp(3.*(-1.+self.aa_interp)*self.wa)
            omegaL= (1.-self.omega0)*omegaf
            E2 = self.omega0/self.aa_interp**3 + omegaL
            var1 = -3.*Omm + (3.*self.aa_interp*self.wa - 3.*(1.+self.w0+self.wa))*omegaL/E2  #a/E^2 dE^2/da
            mu_gamma = 2./3.*Omm**(gamma-1.)*(Omm**gamma+2.-3.*gamma+(0.5-gamma)*var1)          
            D, dDda = odeint(MG_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, self.aa_interp, mu_gamma, self.omega0, self.w0, self.wa).T  
            f = self.aa*dDda/D
            return D[1:], f[1:]
...
class Linder_gamma_a(MGrowth):
    ...
    def growth_parameters(self, gamma0=0.55, gamma1=0., use_quad=True):
        """
        ...
        """  
        if use_quad:
            f = Omega_m(self.aa[1:], self.omega0, self.w0, self.wa)**(gamma0 + gamma1 * (1.-self.aa[1:])**2/self.aa[1:])
            func = lambda a_: Omega_m(a_, self.omega0, self.w0, self.wa)**(gamma0 + gamma1 * (1.-a_)**2/a_)/a_
            Dini = self.a_start * hyp2f1(1./3, 1., 11./6, (self.omega0 - 1.) / self.omega0 * self.a_start**3)
            # Use quad for maximum accuracy (as in original)
            D = jnp.array([Dini * jnp.exp(quad(func, self.a_start, a_i)[0]) for a_i in self.aa[1:]])
            return D, f  
            ...
            Omm = Omega_m(self.aa_interp, self.omega0, self.w0, self.wa)
            gamma = gamma0+gamma1 * (self.aa_interp+1./self.aa_interp-2.)
            #mu_gamma = 2./3.*Omm**(gamma-1.)*(Omm**gamma+2.-3.*gamma+3.*(gamma-0.5)*Omm) for LCDM background
            A = -3.*(1.+self.w0+self.wa);
            omegaf = self.aa_interp**A*jnp.exp(3.*(-1.+self.aa_interp)*self.wa)
            omegaL= (1.-self.omega0)*omegaf
            ...
            var1 = -3.*Omm + (3.*self.aa_interp*self.wa - 3.*(1.+self.w0+self.wa))*omegaL/E2  #a/E^2 dE^2/da
            mu_gamma = 2./3.*Omm**(gamma-1.)*(Omm**gamma+2.-3.*gamma+(0.5-gamma)*var1)  
            mu_gamma_a = 2./3.*Omm**(gamma-1.)*gamma1*(self.aa_interp-1./self.aa_interp)*jnp.log(Omm) + mu_gamma 
            D, dDda = odeint(MG_D_derivatives, jnp.array([self.a_start, 1.0]), self.aa, self.aa_interp, mu_gamma_a, self.omega0, self.w0, self.wa).T  
            f = self.aa*dDda/D
            return D[1:], f[1:]
