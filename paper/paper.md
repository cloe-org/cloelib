...
For a \\textit{Euclid}-like first data release survey specification (six redshift bins), the computation of all angular power spectra for a 3x2-pt analysis requires approximately 19 microseconds per call, following an initialisation period of 1 second to cache `jit` calculations and accelerate subsequent integral evaluations. Similarly, for full-shape spectroscopic Legendre multipoles, the calculation takes 3 microseconds for 1 redshift bin.
